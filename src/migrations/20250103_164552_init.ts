...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`products_docs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`doc_id\` integer,
  	FOREIGN KEY (\`doc_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`products\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`products_docs_order_idx\` ON \`products_docs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_docs_parent_id_idx\` ON \`products_docs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`products_docs_doc_idx\` ON \`products_docs\` (\`doc_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_products_v_version_docs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`doc_id\` integer,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_products_v_version_docs_order_idx\` ON \`_products_v_version_docs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_docs_parent_id_idx\` ON \`_products_v_version_docs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_docs_doc_idx\` ON \`_products_v_version_docs\` (\`doc_id\`);`)
  await db.run(sql`CREATE TABLE \`_products_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_name\` text,
  	\`version_description\` text,
  	\`version_warehouse_id\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tags_category_idx\` ON \`tags\` (\`category_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_imports_id_idx\` ON \`payload_locked_documents_rels\` (\`imports_id\`);`)
